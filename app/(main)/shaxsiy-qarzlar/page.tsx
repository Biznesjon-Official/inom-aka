...
export default function ShaxsiyQarzlarPage() {
  ...
  const [addForm, setAddForm] = useState({ name: '', phone: '', amount: '', note: '', categoryId: '', customerId: '' })
  ...
  const fetchUstalar = useCallback(async () => {
    ...
    if (res.ok) setUstalar(await res.json())
  }, [])
  ...
  const payableTotal = filtered.filter(d => d.status === 'active' && d.direction === 'payable').reduce((s, d) => s + d.remainingAmount, 0)
  ...
  async function handleAdd() {
    ...
    const res = await fetch('/api/personal-debts', {
      ...
      body: JSON.stringify({
        ...
        customer: addForm.customerId || undefined,
        amount: num,
    ...
    setAddDialog(false)
    setAddForm({ name: '', phone: '', amount: '', note: '', categoryId: '', customerId: '' })
    fetchDebts()
  }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-bold text-slate-800 flex items-center gap-2">
            <Wallet className="w-5 h-5" />Shaxsiy qarzlar
          </h1>
          {status === 'active' && payableTotal > 0 && (
            <p className="text-sm text-slate-500 mt-1">Jami qarz: <span className="font-bold text-red-600">{formatPrice(payableTotal)}</span></p>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <Card className="border-0 shadow-sm col-span-2 md:col-span-1">
            <CardContent className="p-3">
              ...
              <div className="text-lg font-bold text-red-600">{formatPrice(payableTotal)}</div>
            </CardContent>
          </Card>
      ...
      <Dialog open={addDialog} onOpenChange={setAddDialog}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              <Label>Usta (cashback uchun)</Label>
              <Select value={addForm.customerId || 'none'} onValueChange={v => setAddForm(f => ({ ...f, customerId: v === 'none' ? '' : v }))}>
                <SelectTrigger><SelectValue placeholder="Tanlang (ixtiyoriy)" /></SelectTrigger>
                <SelectContent>
                  <SelectItem value="none">Ustasiz</SelectItem>
                  {ustalar.map(u => (
                    <SelectItem key={u._id} value={u._id}>
                      {u.name} {u.cashbackPercent > 0 ? `(${u.cashbackPercent}%)` : ''}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
